...
class _OverviewTabState extends ConsumerState<OverviewTab> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.read(gameProvider.notifier).loadCheckInStatus();
    });
  }
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          // 출석체크 패널
          _CheckInPanel(
            checkInStatus: gameState.checkInStatus,
            onCheckIn: () async {
...
class _CheckInPanel extends StatelessWidget {
        ...
        children: [
          ...
          Row(
            ...
            children: List.generate(7, (index) {
              final isChecked = status.weekDays[index];
                  ...
                  children: [
                    ...
                    Text(
                      status.canCheckIn
                          ...
                          : '내일 보상 (${status.rewardHours}시간 생산량)',
                      style: const TextStyle(
                        color: AppColors.textPrimary,
                        fontSize: 11,
                        fontWeight: FontWeight.w500,
          ...
          SizedBox(
            width: double.infinity,
            child: ElevatedButton(
              onPressed: status.canCheckIn ? onCheckIn : null,
              style: ElevatedButton.styleFrom(
                backgroundColor: status.canCheckIn ? AppColors.positive : AppColors.surface,
                foregroundColor: status.canCheckIn ? Colors.white : AppColors.textMuted,
                padding: const EdgeInsets.symmetric(vertical: 12),
              ...
              child: Row(
                ...
                children: [
                  Icon(
                    status.canCheckIn ? Icons.touch_app : Icons.check_circle,
                    size: 18,
                  ),
                  const SizedBox(width: 8),
                  Text(
                    status.canCheckIn ? '출석체크' : '오늘 출석 완료',
                    style: const TextStyle(
                      fontSize: 14,
                      fontWeight: FontWeight.w600,
                    ),
  ...
  bool _isToday(int dayIndex) {
    ...
    // 월=0, 화=1, ..., 일=6
    final todayIndex = now.weekday - 1; // DateTime.weekday: 1=월, 7=일
