...
class AllianceChatState {
  ...
  AllianceChatState copyWith({
    ...
    return AllianceChatState(
      messages: messages ?? this.messages,
      isConnected: isConnected ?? this.isConnected,
      isLoading: isLoading ?? this.isLoading,
      userCount: userCount ?? this.userCount,
      error: error,
      allianceTag: allianceTag ?? this.allianceTag,
      hasAlliance: hasAlliance ?? this.hasAlliance,
    );
  }
}

class AllianceChatNotifier extends StateNotifier<AllianceChatState> {
  final SocketService _socketService;
  final String? _currentUserId;

  AllianceChatNotifier(this._socketService, this._currentUserId) : super(AllianceChatState()) {
    _setupListeners();
  }

  void _setupListeners() {
    _socketService.allianceChatHistoryStream.listen((messages) {
      state = state.copyWith(messages: messages, isLoading: false, isConnected: true);
    });

    _socketService.allianceChatMessageStream.listen((message) {
      final updatedMessages = [...state.messages, message];
      if (updatedMessages.length > 50) {
        updatedMessages.removeAt(0);
      }
    ...
    _socketService.allianceUserCountStream.listen((count) {
      state = state.copyWith(userCount: count);
    });

    _socketService.allianceChatJoinedStream.listen((data) {
      state = state.copyWith(
        isConnected: true,
        isLoading: false,
    ...
    _socketService.allianceChatErrorStream.listen((error) {
      state = state.copyWith(
        error: error,
        isLoading: false,
        hasAlliance: error.contains('가입') ? false : state.hasAlliance,
      );
    });
  }
  ...
  void joinChat() {
    state = state.copyWith(isLoading: true, error: null);
    _socketService.joinAllianceChat();
  }

  void leaveChat() {
    _socketService.leaveAllianceChat();
    state = state.copyWith(isConnected: false);
  }

  void sendMessage(String message) {
    if (message.trim().isNotEmpty) {
      _socketService.sendAllianceChatMessage(message);
    }
  }
...
final allianceChatProvider = StateNotifierProvider<AllianceChatNotifier, AllianceChatState>((ref) {
  final socketService = ref.watch(socketServiceProvider);
  final authState = ref.watch(authProvider);
  return AllianceChatNotifier(socketService, authState.user?.id);
});

// 연합 관련 코드는 alliance_provider.dart 사용
