/**
 * 기존 유저 데이터를 Planet 컬렉션으로 마이그레이션하는 스크립트
 * 
 * 실행 방법:
 * npx ts-node src/scripts/migrate-planets.ts
 * 
 * 또는 빌드 후:
 * node dist/scripts/migrate-planets.js
 */
...
async function migrate() {
  ...
  const planetModel = app.get<Model<PlanetDocument>>(getModelToken(Planet.name));
    ...
    try {
      ...
      const planet = new planetModel({
        ...
        fleet: {
          ...
          colonyShip: (user.fleet as any)?.colonyShip || 0,
        },
        defense: {
